...
<table width=666 cellspacing=5 cellpadding=0 border=0 align=Default>
  <tr>
    <td width=41><IMG SRC="imagens/lo.jpg"></td>
...
<td class=cp valign=top width=7 height=12><img height=12 src=imagens/1.png width=1 border=0></td><td class=cp valign=top align=right width=180 height=12> 
  <span class="campo">
  <?php 
  $valores = str_replace(",", ".",$dadosboleto["valor_boleto"]);	
...
<?php echo $dadosboleto["instrucoes5"]; ?></FONT><br><br> 
</span></td>
...
<button type="button" id="DM" onClick="javascript:window.print(); window.close();" style="border:0px;" onMouseOver="this.style.opacity='1'" onMouseOut="this.style.opacity='0.5'">Imprimir Página Completa</button>
</BODY></HTML>
<script language="Javascript">
// Função que fecha com a Tecla :)
function FechaTecla() {
 var Tecla_Code = event.keyCode;
 if (Tecla_Code == 13) { // Enter
  window.self.close();
 } else if (Tecla_Code == 27) { // Esc
  window.self.close();
 } // e assim vai
}


// Ativa o teclado ao Clique :)
window.document.onkeydown = FechaTecla;
</script>